...
contract ERC721Bid is OwnableUpgradeable, PausableUpgradeable, ERC721BidStorage, MinimalForwarderUpgradeable, IERC721Receiver, ERC2771ContextUpgradeable {
    using SafeMath for uint256;
    using Address for address;

    function initialize(address _theRocksToken, uint256 _ownerCutPerMillion) public initializer {
        ...
        theRocksToken = IERC20(_theRocksToken);
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        if (ownerCutPerMillion > 0) {
            ...
            require(
                theRocksToken.transferFrom(bidder, owner(), saleShareAmount),
                "Transfering the cut to the bid contract owner failed"
            );
        }

        // Transfer TheRocks from bidder to seller
        require(
            theRocksToken.transferFrom(bidder, _from, price.sub(saleShareAmount)),
            "Transfering TheRocks to owner failed"
        );
    ...
    * to use bidder TheRocks on his belhalf
    ...
    */
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(
            theRocksToken.balanceOf(_bidder) >= _amount,
            "Insufficient funds"
        );
        require(
            theRocksToken.allowance(_bidder, address(this)) >= _amount,
            "The contract is not authorized to use TheRocks on bidder behalf"
        );
    }
